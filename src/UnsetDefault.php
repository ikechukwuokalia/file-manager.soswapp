...
try {
  Helper\setting_unset_file_default($file->id);
} catch (\Exception $e) {
  echo \json_encode([
    "status" => "4.1",
...
// if (!$database->query("DELETE FROM `{$file_db}`.`file_default` WHERE file_id = {$file->id}")) {
//   $do_errors = [];
//   $more_errors = (new InstanceError($database,true))->get('query',true);
